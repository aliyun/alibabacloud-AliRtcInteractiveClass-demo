...
-(void)initUI {
    ...
    UIImage *image = [CommonUtils imageWithColor:[[UIColor whiteColor]colorWithAlphaComponent:0]];
    [self.navigationController.navigationBar setBackgroundImage:image forBarMetrics:UIBarMetricsDefault];
    [self.navigationController.navigationBar setShadowImage:image];
    
    [_loginBtn setBackgroundImage:[CommonUtils imageWithColor:[CommonUtils colorWithHex:0x013EBE]] forState:UIControlStateNormal];
    
    [_loginBtn setBackgroundImage:[CommonUtils imageWithColor:[CommonUtils colorWithHex:0xB6C5E9]] forState:UIControlStateDisabled];
    ...
    NSAttributedString *roomPlaceHolder = [[NSAttributedString alloc] initWithString:@"请输入教室编号" attributes:
                                           @{NSForegroundColorAttributeName:[CommonUtils colorWithHex:0x262626],
...
-(void)requestToken:(NSString *)channel{
    __weak typeof(self)weakSelf = self;
    //判断房间人数
    [NetworkManager GET:@"describeChannelUsers" parameters:@{@"channelId":channel} completionHandler:^(NSString * _Nullable errString, NSDictionary * _Nullable resultDic) {
...
- (IBAction)onLogin:(UIButton *)sender {
    if (_nameTextField.text.length<=0) {
        [CommonUtils showHud:@"请输入用户名" inView:self.view];
        return;
    }
    if (_roomTextField.text.length != 6) {
        [CommonUtils showHud:@"请输入6位房间号" inView:self.view];
        return;
    }
    sender.enabled = NO;
